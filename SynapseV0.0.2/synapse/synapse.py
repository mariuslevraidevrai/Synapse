...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⣿⢿⠙⠾⠿⠿⠟⠋⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠸⠿⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀""")
    print("This is Synapse v0.0.2, built for x86_64")


class NeuralNetwork:
    def __init__(self, layers, activation='sigmoid', lr=0.001, normalize=True, task='regression',clipValue=1.0):
        self.clip = clipValue
        self.task = task
    ...
    def save(self, path):
        ...
        if not os.path.exists("models/"):
            os.mkdir("models")
        np.save("models/"+path, data)
    @classmethod
    def load(cls, path):
        path = "models/"+path+".npy"
    ...
    def backward(self, y, output):
        ...
        for i in reversed(range(len(self.weights))):
            ...
            weightsGradient = np.dot(aPrev.T, error)
            biasesGradient = np.sum(error, axis=0, keepdims=True)

            self.weights[i] -= self.lr * weightsGradient
            self.biases[i]  -= self.lr * biasesGradient

            if i > 0:
                error = np.dot(error, self.weights[i].T)
                error *= self.activationDerivative(self.activations[i])
                error = np.clip(error, -self.clip, self.clip)
